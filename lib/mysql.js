...
/*
 *  YOU SHALL PAD YOUR MILLISECONDS OTHERWISE THIS
 *
 *  2015-05-29T13:44:19.030Z*
 *
 *  BECOMES
 *
 *  2015-05-29 13:44:19.3
 *
 *  and that .3 actually means .300
 *
 */
...
function dateToMysql(val) {
    ...
    var millisPad = '000';
    var millis = val.getUTCMilliseconds().toString();
    var mysqlDate = val.getUTCFullYear() + '-' +
    fillZeros(val.getUTCMonth() + 1) + '-' +
    fillZeros(val.getUTCDate()) + ' ' +
    fillZeros(val.getUTCHours()) + ':' +
    fillZeros(val.getUTCMinutes()) + ':' +
    fillZeros(val.getUTCSeconds()) + '.' +
    millisPad.substring(0, millisPad.length - millis.length) + millis;
...
MySQL.prototype.toColumnValue = function(prop, val) {
  ...
  //allow the date coercion to be skipped
  //in order to use sub millisecond precision
  //date strings
  ...
  if (prop) {
    switch (prop.type.name) {
      ...
      case 'Date':
        ...
        val.setMinutes(val.getMinutes() - offset);
